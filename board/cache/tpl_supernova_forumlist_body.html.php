<?php if (!defined('IN_PHPBB')) exit; ?><div id="forumlist">
<?php $_forumrow_count = (isset($this->_tpldata['forumrow'])) ? sizeof($this->_tpldata['forumrow']) : 0;if ($_forumrow_count) {for ($_forumrow_i = 0; $_forumrow_i < $_forumrow_count; ++$_forumrow_i){$_forumrow_val = &$this->_tpldata['forumrow'][$_forumrow_i]; if (( $_forumrow_val['S_IS_CAT'] && ! $_forumrow_val['S_FIRST_ROW'] ) || $_forumrow_val['S_NO_CAT']) {  ?>
     ...
     </div><!-- sn-category-forumID --><?php } if ($_forumrow_val['S_IS_CAT'] || $_forumrow_val['S_FIRST_ROW'] || $_forumrow_val['S_NO_CAT']) {  ?>

    <div id="sn-category-<?php echo $_forumrow_val['FORUM_ID']; ?>" class="iewidth">
		<div class="sn-cat-header">
        	...
        	<ul class="topiclist">
				<li class="header">
					<dl class="icon">
                        	...
                        	<dd style="width: 55.2%;"><div class="cat-name">
            					<span><?php if ($_forumrow_val['S_IS_CAT']) {  ?><a href="<?php echo $_forumrow_val['U_VIEWFORUM']; ?>"><?php echo $_forumrow_val['FORUM_NAME']; ?></a><?php } else { echo ((isset($this->_rootref['L_FORUM'])) ? $this->_rootref['L_FORUM'] : ((isset($user->lang['FORUM'])) ? $user->lang['FORUM'] : '{ FORUM }')); } ?></span>
           					</div></dd>
              
						<dd class="topics sn-cat-header-label"><?php echo ((isset($this->_rootref['L_TOPICS'])) ? $this->_rootref['L_TOPICS'] : ((isset($user->lang['TOPICS'])) ? $user->lang['TOPICS'] : '{ TOPICS }')); ?></dd>
						<dd class="posts sn-cat-header-label"><?php echo ((isset($this->_rootref['L_POSTS'])) ? $this->_rootref['L_POSTS'] : ((isset($user->lang['POSTS'])) ? $user->lang['POSTS'] : '{ POSTS }')); ?></dd>
						<dd class="lastpost sn-cat-header-label"><span><?php echo ((isset($this->_rootref['L_LAST_POST'])) ? $this->_rootref['L_LAST_POST'] : ((isset($user->lang['LAST_POST'])) ? $user->lang['LAST_POST'] : '{ LAST_POST }')); ?></span></dd>
                        <dd class="sn-forumlist-controls">
                        	<?php if ($this->_tpldata['DEFINE']['.']['SN_DRAGGABLE_CATEGORIES']) {  ?><div class="sn-move-icon" <?php if ($this->_rootref['SCRIPT_NAME'] == ('viewforum')) {  ?>style="display: none;"<?php } ?>></div><?php } ?>
                ...
                <div class="sn-cat-row" id="sn-row-<?php echo $_forumrow_val['FORUM_ID']; ?>">
			<ul class="topiclist forums">
	<?php } if (! $_forumrow_val['S_IS_CAT']) {  ?>

		<li class="row">
			<dl class="icon sn-index-icon" style="background-image: url(<?php echo $_forumrow_val['FORUM_FOLDER_IMG_SRC']; ?>); background-repeat: no-repeat; <?php if ($this->_rootref['SCRIPT_NAME'] == ('index') || ('viewforum')) {  ?>padding-left: 40px;<?php } ?>">
				...
				<?php if ($this->_rootref['S_ENABLE_FEEDS'] && $_forumrow_val['S_FEED_ENABLED']) {  ?><a class="feed-icon-forum" title="<?php echo ((isset($this->_rootref['L_FEED'])) ? $this->_rootref['L_FEED'] : ((isset($user->lang['FEED'])) ? $user->lang['FEED'] : '{ FEED }')); ?> - <?php echo $_forumrow_val['FORUM_NAME']; ?>" href="<?php echo (isset($this->_rootref['U_FEED'])) ? $this->_rootref['U_FEED'] : ''; ?>?f=<?php echo $_forumrow_val['FORUM_ID']; ?>"><img src="<?php echo (isset($this->_rootref['T_THEME_PATH'])) ? $this->_rootref['T_THEME_PATH'] : ''; ?>/images/feed.gif" alt="<?php echo ((isset($this->_rootref['L_FEED'])) ? $this->_rootref['L_FEED'] : ((isset($user->lang['FEED'])) ? $user->lang['FEED'] : '{ FEED }')); ?> - <?php echo $_forumrow_val['FORUM_NAME']; ?>" /></a><?php } if ($_forumrow_val['FORUM_IMAGE']) {  ?><span class="forum-image"><?php echo $_forumrow_val['FORUM_IMAGE']; ?></span><?php } ?>
					...
					<span class="sn-forum-description"><?php echo $_forumrow_val['FORUM_DESC']; ?></span>
				...
				<?php } else if (! $_forumrow_val['S_IS_LINK']) {  ?>
					...
					<dd class="topics"><span class="sn-big-stat"><?php echo $_forumrow_val['TOPICS']; ?></span> <dfn><?php echo ((isset($this->_rootref['L_TOPICS'])) ? $this->_rootref['L_TOPICS'] : ((isset($user->lang['TOPICS'])) ? $user->lang['TOPICS'] : '{ TOPICS }')); ?></dfn></dd>
					<dd class="posts"><span class="sn-big-stat"><?php echo $_forumrow_val['POSTS']; ?></span> <dfn><?php echo ((isset($this->_rootref['L_POSTS'])) ? $this->_rootref['L_POSTS'] : ((isset($user->lang['POSTS'])) ? $user->lang['POSTS'] : '{ POSTS }')); ?></dfn></dd>
    ...
    </div><!-- sn-category-forumID --><?php } }} else { ?>
