...
<?php } $_postrow_count = (isset($this->_tpldata['postrow'])) ? sizeof($this->_tpldata['postrow']) : 0;if ($_postrow_count) {for ($_postrow_i = 0; $_postrow_i < $_postrow_count; ++$_postrow_i){$_postrow_val = &$this->_tpldata['postrow'][$_postrow_i]; if ($_postrow_val['S_FIRST_UNREAD']) {  ?><a id="unread"></a><?php } ?>
	...
	<div id="p<?php echo $_postrow_val['POST_ID']; ?>" class="post <?php if (($_postrow_val['S_ROW_COUNT'] & 1)  ) {  ?>bg1<?php } else { ?>bg2<?php } if ($_postrow_val['S_UNREAD_POST']) {  ?> unreadpost<?php } if ($_postrow_val['S_POST_REPORTED']) {  ?> reported<?php } if (! $this->_tpldata['DEFINE']['.']['SN_PROFILES_ON_LEFT']) {  if ($_postrow_val['S_ONLINE'] && ! $_postrow_val['S_IGNORE_POST']) {  ?> online<?php } } ?>">
		<div class="inner"><span class="corners-top"><span></span></span>

		<div class="postbody" <?php if ($this->_tpldata['DEFINE']['.']['SN_PROFILES_ON_LEFT']) {  ?>style="float: right;"<?php } ?>>
		...
		<?php if (! $_postrow_val['S_IGNORE_POST']) {  ?>
			...
			<dl class="postprofile <?php if ($_postrow_val['S_ONLINE'] && ! $_postrow_val['S_IGNORE_POST'] && $this->_tpldata['DEFINE']['.']['SN_PROFILES_ON_LEFT']) {  ?>online<?php } ?>" id="profile<?php echo $_postrow_val['POST_ID']; ?>" <?php if ($this->_tpldata['DEFINE']['.']['SN_PROFILES_ON_LEFT']) {  ?>style="float: left; border-left: none; border-right: 1px solid #FFFFFF; margin-top: -5px;"<?php } ?>>
			<dt <?php if ($this->_tpldata['DEFINE']['.']['SN_PROFILES_ON_LEFT']) {  ?>style="margin-top: 10px;"<?php } ?>>
