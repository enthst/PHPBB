...
<?php } if ($this->_rootref['S_FORUM_RULES']) {  ?>
	...
	<div class="rules">
		...
		<?php } else { ?>
			...
			<span class="rules-header"><?php echo ((isset($this->_rootref['L_FORUM_RULES'])) ? $this->_rootref['L_FORUM_RULES'] : ((isset($user->lang['FORUM_RULES'])) ? $user->lang['FORUM_RULES'] : '{ FORUM_RULES }')); ?></span>
        ...
        <div class="search-box">
            ...
            <fieldset>
                <div class="search-box-inner">
                    ...
                    <input class="inputbox search tiny" type="text" name="keywords" id="keywords" size="20" value="<?php echo ((isset($this->_rootref['LA_SEARCH_TOPIC'])) ? $this->_rootref['LA_SEARCH_TOPIC'] : ((isset($this->_rootref['L_SEARCH_TOPIC'])) ? addslashes($this->_rootref['L_SEARCH_TOPIC']) : ((isset($user->lang['SEARCH_TOPIC'])) ? addslashes($user->lang['SEARCH_TOPIC']) : '{ SEARCH_TOPIC }'))); ?>" onclick="if (this.value == '<?php echo ((isset($this->_rootref['LA_SEARCH_TOPIC'])) ? $this->_rootref['LA_SEARCH_TOPIC'] : ((isset($this->_rootref['L_SEARCH_TOPIC'])) ? addslashes($this->_rootref['L_SEARCH_TOPIC']) : ((isset($user->lang['SEARCH_TOPIC'])) ? addslashes($user->lang['SEARCH_TOPIC']) : '{ SEARCH_TOPIC }'))); ?>') this.value = '';" onblur="if (this.value == '') this.value = '<?php echo ((isset($this->_rootref['LA_SEARCH_TOPIC'])) ? $this->_rootref['LA_SEARCH_TOPIC'] : ((isset($this->_rootref['L_SEARCH_TOPIC'])) ? addslashes($this->_rootref['L_SEARCH_TOPIC']) : ((isset($user->lang['SEARCH_TOPIC'])) ? addslashes($user->lang['SEARCH_TOPIC']) : '{ SEARCH_TOPIC }'))); ?>';" />
...
<?php } $_postrow_count = (isset($this->_tpldata['postrow'])) ? sizeof($this->_tpldata['postrow']) : 0;if ($_postrow_count) {for ($_postrow_i = 0; $_postrow_i < $_postrow_count; ++$_postrow_i){$_postrow_val = &$this->_tpldata['postrow'][$_postrow_i]; if ($_postrow_val['S_FIRST_UNREAD']) {  ?><a id="unread"></a><?php } ?>
	...
	<div id="p<?php echo $_postrow_val['POST_ID']; ?>" class="post <?php if (($_postrow_val['S_ROW_COUNT'] & 1)  ) {  ?>bg1<?php } else { ?>bg2<?php } if ($_postrow_val['S_UNREAD_POST']) {  ?> unreadpost<?php } if ($_postrow_val['S_POST_REPORTED']) {  ?> reported<?php } if ($_postrow_val['S_ONLINE'] && ! $_postrow_val['S_IGNORE_POST']) {  ?> online<?php } ?>">
		...
		<div class="postbody">
			...
			<?php } if ($_postrow_val['BUMPED_MESSAGE']) {  ?><div class="notice"><br /><br /><?php echo $_postrow_val['BUMPED_MESSAGE']; ?></div><?php } if ($_postrow_val['SIGNATURE']) {  ?><div id="sig<?php echo $_postrow_val['POST_ID']; ?>" class="signature"><?php echo $_postrow_val['SIGNATURE']; ?></div><?php } } if ($_postrow_val['THANKS'] && $_postrow_val['THANKS_POSTLIST_VIEW']) {  ?>
		...
		<?php if (! $_postrow_val['S_IGNORE_POST']) {  ?>
			...
			<dt>
				...
				<?php } if ($this->_rootref['S_QR_ACTIVE']) {  if ($this->_rootref['S_QR_COLOR_NICK']) {  if ($_postrow_val['POST_AUTHOR_COLOUR']) {  ?>	
					<a href="<?php echo $_postrow_val['U_POST_AUTHOR']; ?>" onclick="insert_text('[b][color=<?php echo $_postrow_val['POST_AUTHOR_COLOUR']; ?>]<?php echo $_postrow_val['POST_AUTHOR']; ?>[/color][/b], '); return false;"><span style="color: <?php echo $_postrow_val['POST_AUTHOR_COLOUR']; ?>;" class="username-coloured"><?php echo $_postrow_val['POST_AUTHOR']; ?></span></a>
					<?php } else { ?>

					<a href="<?php echo $_postrow_val['U_POST_AUTHOR']; ?>" onclick="insert_text('[b]<?php echo $_postrow_val['POST_AUTHOR']; ?>[/b], '); return false;"><span class="username-coloured"><?php echo $_postrow_val['POST_AUTHOR']; ?></span></a>					
					<?php } } else { ?>

					<a href="<?php echo $_postrow_val['U_POST_AUTHOR']; ?>" onclick="insert_text('[b]<?php echo $_postrow_val['POST_AUTHOR']; ?>[/b], '); return false;"><span style="color: <?php echo $_postrow_val['POST_AUTHOR_COLOUR']; ?>;" class="username-coloured"><?php echo $_postrow_val['POST_AUTHOR']; ?></span></a>			
				<?php } } else { ?>
...
<?php } if ($this->_rootref['S_DISPLAY_ONLINE_LIST']) {  ?>
	...
	<h3 class="dark-header"><?php if ($this->_rootref['U_VIEWONLINE']) {  ?><a href="<?php echo (isset($this->_rootref['U_VIEWONLINE'])) ? $this->_rootref['U_VIEWONLINE'] : ''; ?>"><?php echo ((isset($this->_rootref['L_WHO_IS_ONLINE'])) ? $this->_rootref['L_WHO_IS_ONLINE'] : ((isset($user->lang['WHO_IS_ONLINE'])) ? $user->lang['WHO_IS_ONLINE'] : '{ WHO_IS_ONLINE }')); ?></a><?php } else { echo ((isset($this->_rootref['L_WHO_IS_ONLINE'])) ? $this->_rootref['L_WHO_IS_ONLINE'] : ((isset($user->lang['WHO_IS_ONLINE'])) ? $user->lang['WHO_IS_ONLINE'] : '{ WHO_IS_ONLINE }')); } ?></h3>
	<div class="dark-header-content"><?php echo (isset($this->_rootref['LOGGED_IN_USER_LIST'])) ? $this->_rootref['LOGGED_IN_USER_LIST'] : ''; ?></div>
